...
ssize_t Buffer::readFd(int fd, int *saveErrno)
{
    ...
    // 这里似乎是为了保证（如果socket fd中有特多数据要读的情况下）我们最少能一次从socket fd读64k空间（小补充：最多一次也只能读取128k-1），
    // 虽然前面注释提到过为了效率一次多读点数据，但是不知道为什么一定设置64k？？？？？
    // 如果Buffer缓冲区大小比extrabuf(64k)还小，那就Buffer和extrabuf都用上
    // 如果Buffer缓冲区大小比64k还大或等于，那么就只用Buffer。这意味着，
