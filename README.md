...
# 调试问题
void Channel::handleEvent(Timestamp receiveTime)中tie_的值为false,继续追溯，发现Acceptor::handleRead()未执行,solved

TcpConnection::send(Buffer* buf) 可能有问题，因为http请求只有关闭服务器的时候才能发送给客户端,::write确实调用了，但是客户端就是收不到
